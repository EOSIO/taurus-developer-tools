...
struct fpg_session : connection_callbacks, std::enable_shared_from_this<fpg_session> {
    ...
    abieos_contracts_manager                             contracts_manager;
    ...
    fpg_session(fill_postgresql_plugin_impl* my)
        ...
        if(config->debug_mode) {
            converter.set_abieos_contracts_manager(&contracts_manager);
        }
    }
    ...
    void create_tables() {
        ...
        if(config->debug_mode) {
            t.exec("create table " + converter.schema_name + ".key_value_decoded" +
                   R"( ("block_num" bigint, "present" smallint, "contract" varchar(13), "table" varchar(13), "json" varchar))"); //TODO change last column type to json type
        }
    ...
    void write_table_delta(uint32_t block_num, table_delta&& t_delta, bool bulk) {
        std::visit(
            [&block_num, bulk, this](auto t_delta) {
                ...
                bool is_key_value_delta = t_delta.name == "key_value";
                bool is_account_delta = t_delta.name == "account";
                ...
                for (auto& row : t_delta.rows) {
                    ...
                    if(config->debug_mode) {
                        if(is_account_delta) {
                            string abi_hex = values[4].substr(3);
                            if(!contracts_manager.abieos_set_contract_abi_hex(values[2], abi_hex)) {
                                ilog("Failed to load abi: ${contract_name}", ("contract_name", values[2]));
                            }else {
                                std::vector<string> loaded_contracts = contracts_manager.get_loaded_contract_names();
                                if(std::find(loaded_contracts.begin(), loaded_contracts.end(), values[2]) == loaded_contracts.end()) {
                                    contracts_manager.add_loaded_contract(values[2]);
                                }
                            }
                        } else if(is_key_value_delta) {
                            std::string table_name_hex;
                            bool is_singleton = false;

                            if(values[3][4] == '1') {
                                table_name_hex = values[3].substr(5, 16);
                            } else {
                                //singleton
                                table_name_hex = values[3].substr(11, 16);
                                is_singleton = true;
                            }
                            uint64_t    table_name_uint64_value = name_hex_to_value(table_name_hex);
                            eosio::name table_name(table_name_uint64_value);

                            if(!is_singleton) {
                                string      index_name_hex          = values[3].substr(21, 16);
                                uint64_t    index_name_uint64_value = name_hex_to_value(index_name_hex);
                                eosio::name index_name(index_name_uint64_value);

                                std::string kv_table_primary_index_name = contracts_manager.abieos_get_kv_table_primary_index_name_data(values[2], table_name.to_string());

                                if (index_name.to_string() != kv_table_primary_index_name) {
                                    // secondary index entry - ignore it
                                    continue;
                                }
                            }

                            std::vector<std::string> contract_values;
                            contract_values.push_back(values[0]);
                            contract_values.push_back(values[1]);
                            contract_values.push_back(values[2]);
                            contract_values.push_back(table_name.to_string());

                            string json_value = contracts_manager.abieos_kv_data_to_json(values[2], values[3].substr(3).c_str(), values[4].substr(5).c_str());

                            if(!json_value.empty()) {
                                contract_values.push_back(json_value);
                            } else {
                                contract_values.push_back( "{\"raw\":\"" + values[4].substr(5) + "\"}" );
                            }

                            write_stream(block_num, "key_value_decoded", contract_values);
                        }
                    }
                }
            },
...
void fill_pg_plugin::set_program_options(options_description& cli, options_description& cfg) {
    ...
    clop("fpg-debug-mode", "Decode key-value data on a separate table. Decode action data and return value on transaction traces.");
}
...
void fill_pg_plugin::plugin_initialize(const variables_map& options) {
    try {
        ...
        my->config->debug_mode    = options.count("fpg-debug-mode") ;
    }
    FC_LOG_AND_RETHROW()
}

...
std::string abieos_sql_converter::create_sql_type(
    ...
    if (created_composite_types.count(sql_type) == 0) {
        ...
        for (auto& f : struct_abi_type->fields) {
            ...
            if(debug_mode) {
                if(name == "action" && f_def.name == "data")
                    sub_fields += ", " + quote_name(f_def.name) + " varchar";
                else if(name == "action_trace_v1" && f_def.name == "return_value")
                    sub_fields += ", " + quote_name(f_def.name) + " varchar";
                else
...
std::string abieos_sql_converter::create_sql_type(
    ...
    if (created_composite_types.count(sql_type) == 0) {
        ...
        for (const auto& field : union_fields) {
            if(debug_mode && name=="action_trace" && field.name=="return_value") {
                query += quote_name(field.name) + " varchar,";
            } else {
...
std::string abieos_sql_converter::to_sql_value(eosio::input_stream& bin, const eosio::abi_type& type_ref, field_kind_t field_kind) {
    ...
    if (type->as_struct()) {
        ...
        if(debug_mode) {
            if(type->name == "action") {
                contract_name_debug = values[0].substr(1, values[0].size() - 2);
                action_name_debug = values[1].substr(1, values[1].size() - 2);
                std::string action_type = contracts_manager->abieos_get_action_type(contract_name_debug, action_name_debug);
                if(!action_type.empty()){
                    std::string hex_val = values[3].substr(6);
                    hex_val = hex_val.substr(0, hex_val.size() - 1);
                    std::string json_val = contracts_manager->abieos_hex_data_to_json_pg_copy_safe(contract_name_debug, action_type, hex_val);
                    values[3] = "\"" + json_val + "\"";
                } else{
                    values[3] = "";
                }
        ...
        if(debug_mode){
            if(type->name=="action_trace" && values.size() >= 13){
                std::string action_return_type = contracts_manager->abieos_get_action_type_result(contract_name_debug, action_name_debug);
                if(!action_return_type.empty()){
                    std::string hex_val = values[12].substr(6);
                    hex_val = hex_val.substr(0, hex_val.size() - 1);
                    std::string json_val = contracts_manager->abieos_hex_data_to_json_pg_copy_safe(contract_name_debug, action_return_type, hex_val);
                    values[12] = "\"" + json_val + "\"";
                }else{
                    values[12] = "";
                }
...
void abieos_sql_converter::set_abieos_contracts_manager(abieos_contracts_manager *ptr) {
    contracts_manager = ptr;
    debug_mode = true;
}
